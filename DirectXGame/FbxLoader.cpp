...
void FbxLoader::ParseSkin(Model* model, FbxMesh* fbxMesh)
{
	...
	for (int i = 0; i < vertices.size(); i++)
	{
		...
		int weightArreayIndex = 0;
		// 降順ソート済みのウェイトリストから
		for (auto& weightSet : weightList)
		{
			// 頂点データに書き込み
			vertices[i].boneIndex[weightArreayIndex] = weightSet.index;
			vertices[i].boneWeight[weightArreayIndex] = weightSet.weight;
			// 4つに達したら終了
			if (++weightArreayIndex >= Model::MAX_BONE_INDICES)
			{
				float weight = 0.0f;
				// 2番目以降のウェイトを合計
